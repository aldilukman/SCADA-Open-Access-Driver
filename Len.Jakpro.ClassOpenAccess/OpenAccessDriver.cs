...
namespace Len.Jakpro.ClassOpenAccess
{
    ...
    public class OpenAccessDriver
    {
        ...
        public void ConnectToAudioServer(StringArray serverAddresses, KeyValueMap configItems)
        {
            ...
            catch (Exception Swigexception)
            {
                _logger.Error(DateTime.Now.ToString("dd-MM-yyyy : hh:mm:ss : ") + string.Concat(Swigexception.StackTrace, Swigexception.Message));
                return;
            }
            Thread.Sleep(3000);
            ...
            if (_AudioServer.isAudioConnected())
            {
                ...
                Console.WriteLine("time stasion :" + timeStation);
                string timeStationNext = GetScheduledArrival(1004, 2);
                Console.WriteLine("time stasion Next :" + timeStationNext);
