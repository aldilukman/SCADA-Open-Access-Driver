...
namespace NetspireSDKTester
{
	...
	{
		// Global
		public const int gGUIVersionNumber = 338;
		...
		private void timerDeviceStates_Tick(object sender, EventArgs e)
		{
			try
			{
				...
				for (short i = 0; i < devStateArray.Count; i++)
				{
					...
					netspire.KeyValueMap supplementaryFields = thisDevice.getSupplementaryFields();
					netspire.StringArray supplementaryFieldsAllKeys = supplementaryFields.get_all_keys();
					for (int j = 0; j < supplementaryFieldsAllKeys.Count; j++)
					{
						string key = supplementaryFieldsAllKeys.ElementAt(j);
						thisDevice_supplementaryFieldsStr += "[" + key + "," + supplementaryFields.get_value(key) + "] ";
					}
		...
		/*******************************************************************************************************/
		private void buttonGetPaZoneMembers_Click(object sender, EventArgs e)
		{
			addNewDebugMessage(EventType.PA_EVENT, "");
			try
			{
				if (gAudioServer == null || !gAudioServer.isAudioConnected())
				{
					addNewDebugMessage(EventType.PA_EVENT, "buttonGetPaZoneMembers_Click(): Not connected to AS");
					return;
				}

				if (listBoxPaZones.SelectedItems.Count == 0)
				{
					addNewDebugMessage(EventType.PA_EVENT, "buttonGetPaZoneMembers_Click(): No Audio Sinks selected");
					return;
				}

				// loop through selected paZones and get members
				netspire.PaZoneArray allPaZonesFromAS = gAudioServer.getPAController().getPaZones();
				for (int z = 0; z < listBoxPaZones.SelectedItems.Count; z++)
				{
					String selectedPaZoneId = listBoxPaZones.SelectedItems[z].ToString();
					for (int zoneIndex = 0; zoneIndex < allPaZonesFromAS.Count; zoneIndex++)
					{
						netspire.PaZone thisPaZone = allPaZonesFromAS.ElementAt(zoneIndex);
						if (thisPaZone.id == selectedPaZoneId)
						{
							netspire.PaSinkArray thisPaZoneMembers = thisPaZone.getMembers();
							String msg = "PaSink members for PaZoneId: " + selectedPaZoneId + "\n\n";
							if (thisPaZoneMembers.Count > 0)
							{
								for (int sinkIndex = 0; sinkIndex < thisPaZoneMembers.Count; sinkIndex++)
								{
									msg += ( ((sinkIndex == 0) ? "" : ", ") + thisPaZoneMembers.ElementAt(sinkIndex).id );
								}
							}
							else
							{
			...
			{
				addNewDebugMessage(EventType.PA_EVENT, "buttonGetPaZoneMembers_Click(): exception caught: " + excep.Message);
			}
		}
			...
			try
			{
				...
				for (int i = 0; i < listBoxPaSinks.SelectedItems.Count; i++)
				{
					...
					for (int z = 0; z < sinkListFromAS.Count(); z++)
					{
						...
						// found PaSink
						if (thisPaSink.type == netspire.PaSink.Type.NETSPIRE_DCI2 || thisPaSink.type == netspire.PaSink.Type.NETSPIRE_VCU4)
						{
							thisPaSink.reset();
		...
		* WARATAH OA0979 - Rolling Stock specific functions
		...
		private void getDynamicConfigurationToolStripMenuItem_Click(object sender, EventArgs e)
		{
			try
			{
				...
				netspire.StringArray dynamicConfigurationAllKeys = dynamicConfiguration.get_all_keys();
				string msgForUser = "";
				for (int i = 0; i < dynamicConfigurationAllKeys.Count; i++)
				{
					if (msgForUser.Length != 0)
					{
						msgForUser += "\n";
					}
					string key = dynamicConfigurationAllKeys.ElementAt(i);
					msgForUser += (key + " = " + dynamicConfiguration.get_value(key));
				}
			...
			try
			{
				...
				if (dynamicConfigKeyValueString.Length != 0)
				{
					...
					if (dynamicConfigKeyValueTokens.Length == 2)
					{
						...
						userDefinedDynamicConfig.set(dynamicConfigKeyValueTokens[0], dynamicConfigKeyValueTokens[1]);
		...
		private void connectToKLMonorailToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			netspire.KeyValueMap configItems = new netspire.KeyValueMap();
			//configItems.Add("NETSPIRE_SDK_SOCKET_PORT", "20770");
			configItems.set("NETSPIRE_SDK_HOST_DST_LIST", "2013007,2013008");   // required as KL-Monorail is fixed + rolling-stock
		...
		private void connectToHKToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			netspire.KeyValueMap configItems = new netspire.KeyValueMap();
			//configItems.Add("NETSPIRE_SDK_SOCKET_PORT", "20770");
			configItems.set("NETSPIRE_SDK_HOST_DST_LIST", "2008001,2008002");   // required as HK is fixed + rolling-stock
		...
		private void connectToSLRToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			// set config items
			netspire.KeyValueMap configItems = new netspire.KeyValueMap();
			//configItems.set("NETSPIRE_SDK_SOCKET_PORT", "20770");
		...
		private void connectToASToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				{
					configItems.set("NETSPIRE_SDK_INDIRECT_CXS_CONNECTION", "");
					configItems.set("NETSPIRE_SDK_USE_CONFIG", "");
				}
				...
				if (connectDialogForm.endPtPortNoStr.Length != 0)
				{
					...
					if (isNum)
					{
						configItems.set("NETSPIRE_SDK_SOCKET_PORT", connectDialogForm.endPtPortNoStr);
					}
					else
					{
				...
				{
					configItems.set("NETSPIRE_SDK_HOST_DST_LIST", connectDialogForm.hostDestinationListStr);
				}

				// set logging if enabled
				configItems.set("NETSPIRE_SDK_SET_LOG_LEVEL", connectDialogForm.logLevel);
				if (connectDialogForm.debugFileName.Length > 0)
				{
					configItems.set("NETSPIRE_SDK_SET_DEBUG_FILE", connectDialogForm.debugFileName);
				}
				if (connectDialogForm.errorFileName.Length > 0)
				{
					configItems.set("NETSPIRE_SDK_SET_ERROR_FILE", connectDialogForm.errorFileName);
				}
		...
		private void buttonWaratahSetCommercialRadioInterrupt_Click(object sender, EventArgs e)
		{
			try
			{
				...
				netspire.KeyValueMap userDefinedDynamicConfig = new netspire.KeyValueMap();
				userDefinedDynamicConfig.set("Commercial_Radio_Interrupt_Ms", textBoxWaratahCommercialRadioInterrupt.Text);
				gAudioServer.setDynamicConfiguration(userDefinedDynamicConfig);
			}
			catch (System.Exception excep)
			{
	...
	public class asObserverClass : netspire.AudioServerObserver
	{
		...
		public override void onDeviceStateChange(netspire.Device device)
		{
			...
			netspire.KeyValueMap supplementaryFields = tmpDevice.getSupplementaryFields();
			netspire.StringArray supplementaryFieldsAllKeys = supplementaryFields.get_all_keys();
			...
			String msg = "asObserverClass.onDeviceStateChange() - " +
				...
				"Supplementary Fields Cnt >" + supplementaryFieldsAllKeys.Count + "<";
			for (int i = 0; i < supplementaryFieldsAllKeys.Count; i++)
			{
				//if (supplementaryFieldMsg.Length > 0)
				string key = supplementaryFieldsAllKeys.ElementAt(i);
				supplementaryFieldMsg += (
					"asObserverClass.onDeviceStateChange() - " +
					"device >" + tmpDevice.getName() + "< supplementary key >" + key + "< value >" + supplementaryFields.get_value(key) + "<");
			}
			if (Form1.gMainForm != null)
			{
				new Thread(delegate ()
