...
#include "CampaignWorkerThread.hpp"
...
public:
	...
	void acknowledgeAlarm(const std::string &endPt, const std::string &operatorName, const std::string &ackDescription);
};
...
public:
	...
	// Specifies the state of the most recent Campaign update request
	enum CampaignUpdateStatus
	{
		CAMPAIGN_UPDATE_NONE = 0,		// No campaign updates initiated or the last update is completed. Please query the campaign version using getCampaignVersion() to verify the expected campaign version is loaded on the device.
		CAMPAIGN_UPDATE_IN_PROGRESS,	// Last campaign update is in progress
		CAMPAIGN_UPDATE_FAILED			// Last campaign update failed.
	};
	...
	**/
	unsigned int playMessage(
		const StringArray &zones,
		const StringArray &visualDevices,
		const Gain &gain,
	...
	**/
	...
	// Updates the campaign stored in the Audio Servers with the specified changeset definition file
	bool updateCampaign(const std::string &_campaignXML, const std::string &_updateXML, const StringArray &_fileList);

	// Returns the current status of the campaign update
	CampaignUpdateStatus getCampaignUpdateStatus();

	// Returns the current version of the campaign
	std::string getCampaignVersion();

	// Returns configuration for the latest version. Configuration items are list of key-value pair.
	KeyValueMap getCampaignConfiguration();

	// Returns the last 100 items of logs
	StringArray getCampaignLogs();
	...
	bool uploadFileToActiveServer(const std::string &sourceFile, const std::string &targetFile);
	void setCampaignStatus(int status, const std::string &reason);
	void addCampaignLog(const std::string &_log);

	// Obsolete methods
	PAController::PaSinkArray getAudioSinks(int sourceId);
...
private:
	...
	CampaignUpdateStatus campaignUpdateStatus;
	std::string campaignUpdateReason;
	StringArray campaignLogItems;
};
