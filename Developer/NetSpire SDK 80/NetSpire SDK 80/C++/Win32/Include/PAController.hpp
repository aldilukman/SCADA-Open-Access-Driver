...
public:
	...
	Message()
	{
		...
		this->pathToRecordedStreamToUpload.clear();
	...
	Message(const Message &messageAnother)
	{
		...
		this->pathToRecordedStreamToUpload = messageAnother.pathToRecordedStreamToUpload;
	...
	void setAudioMessage(const std::vector<unsigned int> &dictionaryItems);
	std::vector<unsigned int> getAudioMessageDictionaryItems();
	void setAudioMessage(const std::string &text, const std::string &language, const std::string &voice, TextEncoding encoding = UTF_8);
	void setAudioMessage(const std::vector<TTSVocalizer> &ttsList);
	...
	void setAudioMessage(const std::string &templateName, const std::map<std::string, std::string> &arguments);
	void setAudioMessage(const std::string &pathToRecordedStreamToUpload, AudioEncoding encoding);
	void setVisualMessage(const std::vector<unsigned int> &dictionaryItems);
	std::vector<unsigned int> getVisualMessageDictionaryItems();
	void setVisualMessage(const std::string &text);
	std::string getVisualMessageText();
	void setVisualMessage(const std::string &_templateName, const std::map <std::string, std::vector<DISPLAY_VALUE_INFO> > &_keyValueList, int validity = -1);
	std::string playAnnouncement(const std::vector<std::string>& zones);
	std::string retrieveAudioMessage();
...
private:
	...
	std::string pathToRecordedStreamToUpload;
...
class AnnouncementRequest : public Message
{
public:
	int id;
	...
	AnnouncementRequest()
	{
		...
		this->zones.clear();
		this->completionCode.clear();
		this->completionMsg.clear();
	}

	AnnouncementRequest(const AnnouncementRequest &reqAnother) : Message(reqAnother)
	{
		this->id = reqAnother.id;
		this->stateText = reqAnother.stateText;
		this->startDateTime = reqAnother.startDateTime;
		this->messageContent = reqAnother.messageContent;
		this->priorityText = reqAnother.priorityText;
		this->completionCode = reqAnother.completionCode;
		this->completionMsg = reqAnother.completionMsg;
	}
};
...
public:
	...
	**/
	...
	virtual void onMessageUpdate(AnnouncementRequest req)
	{
#ifdef OA_DEBUG
	...
	virtual void onMessageDelete(AnnouncementRequest req)
	{
#ifdef OA_DEBUG
...
public:
	...
	*/
	...
	int createSwPaTrigger(int sourceId, int triggerPriority, const std::string& preChime, const std::string& displayParameters);
	...
	**/
	unsigned int playMessage(
		const std::vector<std::string> &zones,
		const std::vector<std::string> &visualDevices,
		const Gain &gain,
		const std::vector<unsigned int> &dictionaryItems,
		const char *visualText,
		bool resumeOnInterruptFlag,
		bool overrideExisting,
		unsigned int validityPeriod,
		unsigned int priority,
	...
	unsigned int playMessage(
		...
		const std::vector<unsigned int> &dictionaryItems,
		const char *visualText,
		const PlayMessageParams &params
	);
	...
	**/
	...
	void cancelMessage(Message::Type messageType);
...
private:
	...
	std::map<int, AnnouncementRequest> activeAnnouncementRequestList;
