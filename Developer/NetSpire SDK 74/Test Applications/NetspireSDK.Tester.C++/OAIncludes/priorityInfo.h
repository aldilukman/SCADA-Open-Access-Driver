...
typedef enum
{
	PENDING = 0,					// used by requester and parser to indicate that is waiting to be played (init and queue state)
	WAITING_TO_ANNOUNCE,			// 
	ACTIVE,							// all/some target zones are playing requested announcement
	COMPLETED,						// from dispatcher point of view all target zones have played the announcement
	CANCELLED,						// when requester cancels the announcement request
	WAITING_TO_STOP,				//
	INTERRUPTED,					// from dispatcher point of view some/all target zones have started playing another higher priority request
	FAILED,
	PENDING_FILE_CREATE,			// used by AT_TTS when request is sent to resTTS to generate file
	WAITING_PLAYER,					// used by AT_LIVE, AT_STORED_DATA, AT_TTS, when streaming resource is not available for playback
	SYNC_COMPLETE,					// used by AT_TTS_SAF & AT_STORED_DATA_SAF to indicate to clients to start sending multicast packets for sync playback
	PENDING_FILE_TRANSFER			// used by AT_TTS_SAF & AT_STORED_DATA_SAF when request is sent to sending agent to transfer file to end clients
} AnnouncementState;
